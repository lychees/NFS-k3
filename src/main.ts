...
input.onPress('Digit4', () => {
  if (race.state === 'menu' && !inGarage) startRace('hotpursuit');
});
...
function animate(): void {
  ...
  if (state === 'menu') {
    ...
    if (state === 'countdown') {
      ...
      resolveCarCollisions(mode === 'hotpursuit' ? [...cars, ...pursuit.cars] : cars);
      updateSmoke(dt);
      ...
      if (state === 'racing') {
        ...
        // 玩家冲线/夺冠/被淘汰/被逮捕：结算 + 积分 + 纪录 + 音效
        resultsShown = true;
        hud.hideBanner();
        const earned = bustedRace
          ? 10
